...
sequenceDiagram
    ...
    W->>A: Provide inputs (URL, cert, API key, asset, account)
    A->>SG: mTLS handshake (client certificate)
    A->>SG: GET /service/a2a/v4/A2ARegistrations/RetrievableAccounts
    SG-->>A: List of retrievable accounts with API keys
    A->>A: Match asset_name + account_name
...
2. **An A2A registration** — Created in Safeguard with:
   ...
   - One or more managed accounts added to the registration
...
4. **The A2A API key** — Generated by Safeguard when the A2A registration is created.
...
| `api_token` | Yes | — | A2A API key (registration token) from Safeguard |
...
| `asset_name` | Yes | — | Name of the target asset in Safeguard |
| `account_name` | Yes | — | Name of the account on the asset |
...
| `SAFEGUARD_A2A_TOKEN` | The A2A API key from Safeguard |
...
```yaml
      - name: Get registry credentials
        ...
        with:
          ...
          client_certificate: ${{ secrets.SAFEGUARD_CLIENT_CERT }}
          client_certificate_key: ${{ secrets.SAFEGUARD_CLIENT_KEY }}
          asset_name: 'docker-registry'
          account_name: 'ci-push'
...
| **API token masking** | The A2A API key is masked immediately on action startup. |
...
SAFEGUARD_A2A_TOKEN=your-api-key
...
| Error | Cause | Fix |
|-------|-------|-----|
| `Authentication failed (HTTP 401)` | Invalid or expired A2A API key | Verify the API key in Safeguard Admin → A2A Registrations |
...
| `No retrievable accounts found` | A2A registration has no credential retrieval accounts | Add accounts to the A2A registration in Safeguard |
| `No matching account found` | Asset/account name mismatch | Check spelling; the error message lists available accounts |
